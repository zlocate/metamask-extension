...
class BnAsDecimalInput extends LocaleComponent {
...
BnAsDecimalInput.prototype.constructWarning = function () {
  ...
  if (min && max) {
    message += this.t('betweenMinAndMax', [`${newMin} ${suffix}`, `${newMax} ${suffix}`])
  } else if (min) {
    message += this.t('greaterThanMin', [`${newMin} ${suffix}`])
  } else if (max) {
    message += this.t('lessThanMax', [`${newMax} ${suffix}`])
  } else {
