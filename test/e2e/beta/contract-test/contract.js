...
window.addEventListener('message', function (event) {
  ...
  if (!event.data || !event.data.type || event.data.type !== 'ETHEREUM_PROVIDER_SUCCESS') { return }
  console.log(`window.firstMessage`, window.firstMessage);
  if (window.firstMessage) {
    ...
    createToken.addEventListener('click', async function (event) {
      ...
      var humanstandardtoken = humanstandardtokenContract.new(
         _initialAmount,
         _tokenName,
         _decimalUnits,
         _tokenSymbol,
