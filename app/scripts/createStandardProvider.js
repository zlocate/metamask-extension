...
export default function createStandardProvider (provider) {
  ...
  const sendSync = provider.send
  provider.send = (payload, callback, beta) => {
    return beta ?
      standardProvider.send(payload.method, payload.params) :
      sendSync.call(provider, payload, callback)
  }
  return provider
}
